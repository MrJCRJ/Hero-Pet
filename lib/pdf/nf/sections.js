...
export const drawTransportadora = (doc, info) => {
  ...
  const rows = [
    [["Transportadora", STR(info?.razao || '—')], ["Quantidade", STR(info?.quantidade != null ? String(info.quantidade) : '—')]],
    [["Razão social", STR(info?.razao || '—')], ["Espécie", STR(info?.especie || '')]],
    [["CPF", formatCpfCnpj(info?.cpf || '')], ["Peso B", STR(info?.pesoB || '—')]],
    [["Placa", STR(info?.placa || '—')], ["Peso L", STR(info?.pesoL || '—')]],
    [["UF", STR(info?.uf || '—')], null],
  ];

  const bottom = drawInfoBoxTwoCols(doc, 'Transportadora', rows, left, y, w, 132);
  doc.y = bottom + 8;
  return bottom;
};
...
export const drawFooter = (doc) => {
  ...
  doc.fontSize(8).fillColor('#6B7280').text(`Nota feita a punho por ${EMITENTE.razao} • CNPJ ${EMITENTE.cnpj} sem nenhum valor fiscal`,

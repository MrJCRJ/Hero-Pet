...
export const drawHeader = (doc, pedido, LOGO_PNG) => {
  ...
  if (LOGO_PNG) {
    try {
      doc.image(LOGO_PNG, left + 8, top - 6, {
        width: 36,
        height: 36,
        fit: [36, 36],
      });
  ...
  const textWidth = 180;
  doc.text(info.join("  •  "), right - textWidth, top, {
    width: textWidth,
    align: "right",
  });
...
export const drawSignature = (doc, startY, drawHeaderFn) => {
  ...
  doc
    .strokeColor("#D1D5DB")
    .moveTo(left, lineY)
    .lineTo(left + colW, lineY)
    .stroke();
  doc
    .strokeColor("#D1D5DB")
    .moveTo(left + colW + 20, lineY)
    .lineTo(right, lineY)
    .stroke();
  ...
  doc
    .strokeColor("#D1D5DB")
    .moveTo(left, lineY2)
    .lineTo(left + colW, lineY2)
    .stroke();
  doc
    .strokeColor("#D1D5DB")
    .moveTo(left + colW + 20, lineY2)
    .lineTo(right, lineY2)
    .stroke();

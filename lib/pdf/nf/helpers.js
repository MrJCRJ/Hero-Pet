...
export const stripDigits = (value = '') => String(value || '').replace(/\D+/g, '');
export const BRL = (n) => Number(n || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
export const DATE = (d) => (d ? new Date(d).toLocaleDateString('pt-BR') : '—');
...
export const composeEnderecoFromViaCep = (viaCepAddr, numeroRaw = '', complementoRaw = '', cepDigits = '') => {
...
export const tryLoadPixQrPng = () => {
  ...
  try {
    const candidates = [
      path.join(process.cwd(), 'public', 'pix-qr.png'),
    ];

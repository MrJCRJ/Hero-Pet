...
export interface AgingFornecedorRow {
  entity_id: number;
  nome: string;
  a_vencer: number;
  vencido_0_30: number;
...
export interface AgingFornecedoresTotais {
  a_vencer: number;
  vencido_0_30: number;
  vencido_31_60: number;
  vencido_61_90: number;
...
export async function fetchContasPagarAgingPorFornecedor(): Promise<{
  fornecedores: AgingFornecedorRow[];
  totais: AgingFornecedoresTotais;
}> {
  const result = await database.query({
    text: `WITH aberto AS (
           ...
           SELECT *
             FROM agg
             ORDER BY total_aberto DESC NULLS LAST
             LIMIT 500`,
  });

  const rows = result.rows as Array<Record<string, unknown>>;
  const fornecedores: AgingFornecedorRow[] = rows.map((r) => ({
  ...
  const totais: AgingFornecedoresTotais = fornecedores.reduce(
    (acc, c) => ({

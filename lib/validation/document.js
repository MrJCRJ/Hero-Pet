// Shared document validation & formatting logic (backend + frontend)
// Centraliza funções antes duplicadas em components/entity/utils.js
...
export function classifyDocument(raw, isPendingFlag = false) {
  ...
  if (isPendingFlag || !digits) return { status: "pending", valid: false, type: null };
  if (digits.length <= 11) {
    if (digits.length < 11) return { status: "provisional", valid: false, type: "CPF" };
    const valid = isValidCPF(digits);
    return { status: valid ? "valid" : "provisional", valid, type: "CPF" };
  }
  if (digits.length < 14) return { status: "provisional", valid: false, type: "CNPJ" };

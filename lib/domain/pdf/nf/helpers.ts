// lib/pdf/nf/helpers.ts
// Helpers compartilhados para geração de NF com PDFKit
import fs from "fs";
import path from "path";

export const STR = (v: unknown, fb = "—"): string =>
  v == null || v === "" ? fb : String(v);
export const stripDigits = (value: string | number = ""): string =>
...
export const formatCep = (raw: string | number = ""): string => {
  const d = stripDigits(String(raw)).slice(0, 8);
  return d ? d.replace(/(\d{5})(\d)/, "$1-$2") : "—";
};

export const formatTelefone = (raw: string | number = ""): string => {
  const d = stripDigits(String(raw)).slice(0, 11);
  if (!d) return "—";
  if (d.length <= 10)
...
export const formatCpfCnpj = (raw: string | number = ""): string => {
  const d = stripDigits(String(raw)).slice(0, 14);
  if (d.length <= 11) {
    return d
      .replace(/(\d{3})(\d)/, "$1.$2")
      .replace(/(\d{3})(\d)/, "$1.$2")
...
export const abbreviateLogradouro = (s: string = ""): string => {
...
export const abbreviateCity = (s: string = ""): string => {
  let out = String(s || "");
...
export const composeEnderecoFromViaCep = (
  viaCepAddr: ViaCepAddr | null | undefined,
  numeroRaw: string | number = "",
  complementoRaw: string | number = "",
  cepDigits: string | number = "",
): string => {
...
export const tryFetchViaCep = async (
  cepDigits: string | number | null | undefined,
  timeoutMs = 1500,
): Promise<ViaCepAddr | null> => {
  const cleanCep = stripDigits(String(cepDigits || "")).slice(0, 8);
  ...
  try {
    ...
    if (resp && resp.ok) {
      const json = (await resp.json()) as ViaCepAddr & { erro?: boolean };
...
export const tryLoadLogoPng = (): string | null => {
  try {
...
export const tryLoadPixQrPng = (): string | null => {
  try {

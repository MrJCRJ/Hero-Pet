...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    if (action === "unpay") return markUnpaid(req, res);
    res.status(400).json({ error: "Ação inválida. Use action=pay ou action=unpay" });
    return;
  }
...
async function markUnpaid(
  ...
  try {
    ...
    if (!(row.rows[0] as Record<string, unknown>).paid_at) {
      await client.query("ROLLBACK");
      res.status(200).json({ ok: true, alreadyUnpaid: true });
      return;
    }
    await client.query({
      text: `UPDATE pedido_promissorias SET paid_at = NULL, updated_at = NOW() WHERE pedido_id = $1 AND seq = $2`,
      values: [id, seq],
    });
    await client.query("COMMIT");
    res.status(200).json({ ok: true });
    ...
    await database.safeRollback(client);
    console.error("POST /pedidos/:id/promissorias/:seq?action=unpay error", e);
    const err = e as { code?: string; message?: string };
    if (isRelationMissing(e))
      res.status(503).json({
        error: "Schema not migrated",

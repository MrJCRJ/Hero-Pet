...
describe("PromissoriasDots", () => {
  ...
  test("render fallback count while vazio e depois carrega dados", async () => {
    const restore = mockFetchSequence([
      ...
      {
        data: [
          { seq: 1, status: "PENDENTE", amount: 10, due_date: "2025-10-05" },
        ],
      },
    ]);

    const { rerender, findByLabelText } = render(
      <PromissoriasDots pedidoId={123} count={3} />,
    );
    ...
    const fallback = await screen.findByLabelText(
      /3 parcela\(s\).*detalhados ainda não carregados/i,
    );

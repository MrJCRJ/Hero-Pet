...
describe("usePedidoTotals", () => {
  it("calcula subtotal, descontos e totalLiquido com desconto aplicado (VENDA)", () => {
    ...
    const { itensRef, tipoRef, numeroPromissoriasRef, freteRef } = buildRefs({
      itens,
    });
  ...
  it("soma frete apenas quando tipo = COMPRA", () => {
    ...
    const refsVenda = buildRefs({
      itens: itensCompra,
      tipo: "VENDA",
      frete: 15,
    });
    const { result: resultVenda } = renderHook(() =>
      usePedidoTotals({ ...refsVenda, setValorPorPromissoria: () => {} }),
    );

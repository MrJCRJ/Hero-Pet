...
describe('GET /api/v1/pedidos/:id/nf', () => {
  test('responde coerente: 200 com PDF ou códigos esperados (400/404/503)', async () => {
    ...
    const hasPdfContentType = String(headers['content-type'] || '').includes('application/pdf');
    const hasDisposition = /attachment; filename="NF-\d+\.pdf"/.test(String(headers['content-disposition'] || ''));
    const hasNonEmptyBody = typeof data?.byteLength === 'number' && data.byteLength > 1000;
    expect(!is200 || (hasPdfContentType && hasDisposition && hasNonEmptyBody)).toBe(true);

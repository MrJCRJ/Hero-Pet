...
function api(path, init = {}) {
  return fetch(`http://localhost:3000${path}`, { ...init, headers: { 'Content-Type': 'application/json', ...(init.headers || {}) } });
}

async function createEntradaLegacy(produtoId, quantidade, valor_unitario = 8) {
  const r = await api('/api/v1/estoque/movimentos', { method: 'POST', body: JSON.stringify({ produto_id: produtoId, tipo: 'ENTRADA', quantidade, valor_unitario }) });
...
async function createCompraLote(produtoId, quantidade, preco = 10) {
  ...
  const body = { tipo: 'COMPRA', partner_entity_id: 1, itens: [{ produto_id: produtoId, quantidade, preco_unitario: preco }] };
...
describe('FIFO debug + migration job', () => {
  ...
  beforeAll(async () => {
    ...
    const er = await api('/api/v1/entities', { method: 'POST', body: JSON.stringify({ name: 'Cliente FIFO Debug', entity_type: 'PJ', document_digits: '', document_status: 'pending', document_pending: false }) });
    await er.json();
    // cria fornecedor (obrigatório para produto)
    const ef = await api('/api/v1/entities', { method: 'POST', body: JSON.stringify({ name: 'Fornecedor FIFO Debug', entity_type: 'PJ', document_digits: '', document_status: 'pending', document_pending: false }) });
    const fornecedor = await ef.json();
    // cria produto simples vinculando fornecedor
    const pr = await api('/api/v1/produtos', { method: 'POST', body: JSON.stringify({ nome: 'Produto FIFO Debug', preco_tabela: 50, fornecedor_id: fornecedor.id }) });
  ...
  test('pedido venda initial legacy -> eligible após compra de lotes -> migrado via job', async () => {
    ...
    const job = await api('/api/v1/pedidos/fifo_migration_job', { method: 'POST', body: JSON.stringify({ limit: 10 }) });

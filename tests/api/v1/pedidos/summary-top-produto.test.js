...
describe("Summary Top Produto Lucro", () => {
  test("Retorna topProdutoLucro coerente com dados", async () => {
    ...
    await venda(cliente, [
      { produto_id: p1.id, quantidade: 3, preco_unitario: 100 },
    ]);
    ...
    await venda(cliente, [
      { produto_id: p2.id, quantidade: 2, preco_unitario: 150 },
    ]);

    const month = yyyyMM(new Date());
    const sum = await fetch(
      `http://localhost:3100/api/v1/pedidos/summary?month=${month}`,
    );
    ...
    expect(Number(json.topProdutoLucro.lucro).toFixed(2)).toBe(
      (
        Number(json.topProdutoLucro.receita) - Number(json.topProdutoLucro.cogs)
      ).toFixed(2),
    );
    // History consistência (cada produto listado tem objeto de history)
    const histMap = new Map(
      json.topProdutosHistory.map((h) => [h.produto_id, h.history]),
    );
    json.topProdutosLucro.forEach((r) => {

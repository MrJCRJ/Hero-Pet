...
describe('Orders highlight - integração', () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn((url) => {
      ...
      if (u.includes('/api/v1/pedidos/123')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve(mockPedido) });
      }
  ...
  test('abre formulário de edição ao detectar highlight', async () => {
    render(<Wrapper><OrdersManager limit={20} /></Wrapper>);

    // Estado de carregando highlight
    expect(await screen.findByText(/Carregando pedido #123/i)).toBeInTheDocument();

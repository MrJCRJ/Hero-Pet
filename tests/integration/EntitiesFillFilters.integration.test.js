...
describe("EntitiesBrowser filtros Endereço/Contato (UI)", () => {
  test("Endereço: completo/parcial/vazio", async () => {
    ...
    for (const f of addrFixtures) {
      expect(
        await screen.findByText(new RegExp(f.name, "i")),
      ).toBeInTheDocument();
    }
    ...
    await waitFor(() => {
      expect(
        screen.getByText(new RegExp(`ADDR_CEP_ONLY_${ts}`, "i")),
      ).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        screen.getByText(new RegExp(`ADDR_EMPTY_${ts}`, "i")),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(new RegExp(`ADDR_CEP_ONLY_${ts}`, "i")),
      ).toBeNull();
  ...
  test("Contato: completo/parcial/vazio", async () => {
    ...
    const contactFixtures = [
      ...
      {
        name: `CONTACT_FULL_${ts}`,
        data: { telefone: "11987654321", email: "ok@example.com" },
      },
      {
        name: `CONTACT_TEL_VALID_ONLY_${ts}`,
    ...
    await waitFor(() => {
      expect(
        screen.getByText(new RegExp(`CONTACT_FULL_${ts}`, "i")),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(new RegExp(`CONTACT_TEL_VALID_ONLY_${ts}`, "i")),
      ).toBeNull();
      expect(
        screen.queryByText(new RegExp(`CONTACT_EMAIL_ONLY_${ts}`, "i")),
      ).toBeNull();
      expect(
        screen.queryByText(new RegExp(`CONTACT_TEL_INVALID_ONLY_${ts}`, "i")),
      ).toBeNull();
      expect(
        screen.queryByText(new RegExp(`CONTACT_EMPTY_${ts}`, "i")),
      ).toBeNull();
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText(new RegExp(`CONTACT_TEL_VALID_ONLY_${ts}`, "i")),
      ).toBeInTheDocument();
      expect(
        screen.getByText(new RegExp(`CONTACT_EMAIL_ONLY_${ts}`, "i")),
      ).toBeInTheDocument();
      expect(
        screen.getByText(new RegExp(`CONTACT_TEL_INVALID_ONLY_${ts}`, "i")),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(new RegExp(`CONTACT_FULL_${ts}`, "i")),
      ).toBeNull();
      expect(
        screen.queryByText(new RegExp(`CONTACT_EMPTY_${ts}`, "i")),
      ).toBeNull();
    ...
    await waitFor(() => {
      expect(
        screen.getByText(new RegExp(`CONTACT_EMPTY_${ts}`, "i")),
      ).toBeInTheDocument();
      expect(
        screen.queryByText(new RegExp(`CONTACT_TEL_VALID_ONLY_${ts}`, "i")),
      ).toBeNull();

...
describe('Products highlight - integração', () => {
  beforeEach(() => {
    global.fetch = jest.fn((url) => {
      ...
      if (u.includes('/api/v1/produtos/55')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve(mockProduto) });
      }
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
    });
  ...
  test('abre modal de edição via highlight', async () => {
    render(<Wrapper><ProductsManager /></Wrapper>);

    expect(await screen.findByText(/Carregando produto #55/i)).toBeInTheDocument();

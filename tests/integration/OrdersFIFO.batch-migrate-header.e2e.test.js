...
describe('OrdersManager botão Migrar FIFO (x) header', () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn(async (url, opts) => {
      ...
      if (u.includes('/api/v1/pedidos/legacy_count')) {
        return { ok: true, json: async () => ({ legacy_count: migratedAll ? 0 : 3 }) };
      }
      if (u.includes('/api/v1/pedidos/migrate_fifo_all') && opts?.method === 'POST') {
        migratedAll = true;
        return { ok: true, json: async () => ({ migrated: 3, remaining_hint: '0 restantes' }) };
      }
  ...
  test('executa migração em massa e some botão', async () => {
    ...
    const batchBtn = await screen.findByRole('button', { name: /Migrar FIFO \(3\)/i });
    ...
    expect(screen.queryByRole('button', { name: /Migrar FIFO \(3\)/i })).toBeNull();

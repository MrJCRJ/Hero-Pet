...
describe("usePedidoItems", () => {
  ...
  test("hidrata itens ao receber editingOrder", () => {
    const editing = {
      itens: [{ produto_id: 10, quantidade: 2, preco_unitario: 5 }],
    };
    const { result, rerender } = renderHook(
      ({ order }) => usePedidoItems(order),
  ...
  test("add, update e remove item", () => {
    const { result } = renderHook(() => usePedidoItems(null));
    act(() => {
      result.current.addItem({ produto_id: 1, quantidade: 3 });
    });
    expect(result.current.itens).toHaveLength(2);
    act(() => {
      result.current.updateItem(1, { quantidade: 5 });
    });

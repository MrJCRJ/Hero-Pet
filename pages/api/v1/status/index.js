...
async function status(request, response) {
  ...
  try {
    ...
    postgresVersion = await database.query("SHOW server_version;");
    const end1 = performance.now();
    timings.first_query = end1 - start1;

    const start2 = performance.now();
    maxConnections = await database.query("SHOW max_connections;");
    const end2 = performance.now();
    timings.second_query = end2 - start2;

    const databaseName = process.env.POSTGRES_DB;
    const start3 = performance.now();
    currentConnections = await database.query({
    ...
    return response.status(503).json({
      ...
      dependencies: {
        database: { status: 'unreachable', error: err.message },
        webserver: {
          status: 'healthy',

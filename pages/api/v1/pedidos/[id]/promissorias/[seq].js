...
async function markPaid(req, res) {
  ...
  try {
    ...
    if (!Number.isFinite(id) || !Number.isFinite(seq)) return res.status(400).json({ error: 'invalid id/seq' });
    await client.query('BEGIN');
    const row = await client.query({ text: `SELECT * FROM pedido_promissorias WHERE pedido_id = $1 AND seq = $2`, values: [id, seq] });
    if (!row.rows.length) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: 'Promissória não encontrada' });
    }
    ...
    await client.query({ text: `UPDATE pedido_promissorias SET paid_at = NOW(), updated_at = NOW() WHERE pedido_id = $1 AND seq = $2`, values: [id, seq] });
    await client.query('COMMIT');

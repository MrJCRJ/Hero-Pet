...
import { stripDigits, tryFetchViaCep, composeEnderecoFromViaCep, tryLoadLogoPng } from 'lib/pdf/nf/helpers';
import { drawPromissoriaCard, PROMISSORIA_CARD_HEIGHT } from 'lib/pdf/promissoria/sections';
...
async function getPromissoriasPDF(req, res) {
  try {
    ...
    if (pedido.tipo !== 'VENDA') return res.status(400).json({ error: 'Promissórias válidas apenas para pedidos de VENDA' });
    if (!pedido.parcelado) return res.status(400).json({ error: 'Pedido não está parcelado em promissórias' });
    ...
    const totalItens = itens.reduce((acc, it) => {
      ...
      const total = Number.isFinite(qtd) && Number.isFinite(preco) ? (qtd * (preco - desc)) : 0;
      return acc + (Number.isFinite(total) ? total : 0);
    }, 0);
    const totalLiquido = (pedido.total_liquido != null ? Number(pedido.total_liquido) : totalItens);

    const n = Number(pedido.numero_promissorias || 1);
    const firstDate = pedido.data_primeira_promissoria || pedido.data_emissao || new Date();
    const baseAmount = Number(pedido.valor_por_promissoria || (n > 0 ? totalLiquido / n : totalLiquido));
    ...
    for (let i = 0; i < Math.max(1, n); i++) {
      ...
      parcelas.push({ seq: i + 1, totalSeq: Math.max(1, n), amount, dueDate: due });
    }
    ...
    res.setHeader('Content-Disposition', `attachment; filename="Promissorias-${pedido.id}.pdf"`);

...
module.exports = plugin(function ({ addComponents, addUtilities, theme }) {
  ...
  const indigo = (colors.indigo && colors.indigo[600]) ? colors.indigo : { 600: '#4f46e5', 700: '#4338ca' };
  const accent = 'var(--color-accent, ' + (indigo[600] || '#4f46e5') + ')';
  const accentHover = 'var(--color-accent-hover, ' + (indigo[700] || '#4338ca') + ')';
  ...
  const surfaces = {
    ...
    '.divider': {
      height: '1px',
      width: '100%',
      background: 'linear-gradient(to right, transparent, var(--color-border) 40%, var(--color-border) 60%, transparent)',
      margin: '1.5rem 0'
    }
  };
  ...
  addUtilities({ '.ring-focus': { boxShadow: 'var(--shadow-focus)' } }, ['focus-visible']);
});

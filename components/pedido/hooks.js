...
import { fetchSaldo as fetchSaldoService, fetchEntities as fetchEntitiesService, fetchProdutos as fetchProdutosService } from './service';
...
export function usePedidoFetchers({ tipo, partnerId }) {
  const fetchEntities = React.useCallback(async (q) => {
    return fetchEntitiesService({ q, tipo });
  }, [tipo]);

  const fetchProdutos = React.useCallback(async (q) => {

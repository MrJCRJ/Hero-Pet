...
export function ToastProvider({ children }) {
  ...
  const basePush = useCallback((channel, message, { type = 'success', timeout = 6000 } = {}) => {
    const id = ++idCounter;
  ...
  const pushAssertive = useCallback((message, opts = {}) => basePush('assertive', message, opts), [basePush]);
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      {/* Canal polite */}
      <div className="fixed z-50 bottom-4 right-4 flex flex-col gap-2 max-w-sm" role="status" aria-live="polite">
        {politeToasts.map(t => (
          <div key={t.id} className={`rounded-md shadow px-4 py-2 text-sm text-white flex items-start gap-3 animate-fade-in-up ${t.type === 'error' ? 'bg-red-600' : t.type === 'warn' ? 'bg-yellow-600' : 'bg-green-600'}`}>
            <span className="flex-1">{t.message}</span>
            <button onClick={() => remove(t.id, 'polite')} className="opacity-70 hover:opacity-100 transition-colors" aria-label="Fechar">×</button>
          </div>
        ))}
      </div>
      {/* Canal assertive (uso para erros críticos) */}
      <div className="fixed z-50 top-4 right-4 flex flex-col gap-2 max-w-sm" role="alert" aria-live="assertive">
        {assertiveToasts.map(t => (
          <div key={t.id} className={`rounded-md shadow px-4 py-2 text-sm text-white flex items-start gap-3 animate-fade-in-up border-2 ${t.type === 'error' ? 'bg-red-700 border-red-300' : t.type === 'warn' ? 'bg-yellow-700 border-yellow-300' : 'bg-green-700 border-green-300'}`}>
            <span className="flex-1">{t.message}</span>
            <button onClick={() => remove(t.id, 'assertive')} className="opacity-70 hover:opacity-100 transition-colors" aria-label="Fechar">×</button>
          </div>
        ))}
      </div>

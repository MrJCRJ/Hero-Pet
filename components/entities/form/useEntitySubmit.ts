    ...
    async ({ form, editingId }) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (res.status === 409) {
            const existingId = data.existing_entity_id;
            const err = new Error(
              data.error ||
                MSG.ENTITY_CONFLICT ||
                "Já existe uma entidade com este documento.",
            ) as Error & { existingEntityId?: number };
            err.existingEntityId = typeof existingId === "number" ? existingId : undefined;
            throw err;
          }
          throw new Error(data.error || `${MSG.GENERIC_ERROR} (${res.status})`);
        }
        return { ok: true };
      } catch (e) {
        const err = e as Error & { existingEntityId?: number };
        const msg = err.message || "Erro desconhecido";
        setError(msg);
        push?.(msg, { type: "error", timeout: 5000, assertive: true });
        return { ok: false, error: msg, existingEntityId: err.existingEntityId };
      } finally {
        setSubmitting(false);
      }
    },

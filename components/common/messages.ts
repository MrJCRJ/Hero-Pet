...
export const MSG = {
  ...
  // Templates dinâmicos
  ORDER_DELETED_SUCCESS: (id: number) => `Pedido #${id} excluído.`,
  ORDER_DELETE_CONFIRM_TITLE: (id: number) => `Excluir Pedido #${id}`,
  ORDER_DELETE_CONFIRM_MESSAGE: (id: number) =>
...
export function msg(
  key: keyof typeof MSG | string,
  fallback?: string
): unknown {
  return (MSG as Record<string, unknown>)[key] ?? fallback ?? key;
}

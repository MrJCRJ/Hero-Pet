...
export interface LineAreaChartPointWithCoords extends LineAreaChartPoint {
  x: number;
  y: number;
}

export interface LineAreaChartStats {
  min: number;
  max: number;
  deltaAbs: number;
  deltaPct: number;
...
export interface LineAreaChartProps {
  ...
  tooltipRender?: (
    point: LineAreaChartPointWithCoords,
    stats: LineAreaChartStats
  ) => React.ReactNode;
  footerRender?: (
    points: LineAreaChartPointWithCoords[],
    stats: LineAreaChartStats
  ) => React.ReactNode;
  valueFormatterLabel?: boolean;
  onHover?: (point: LineAreaChartPointWithCoords | null) => void;
  disableTooltip?: boolean;
  enableCrosshair?: boolean;
  onSelectPoint?: (point: LineAreaChartPointWithCoords) => void;
  selectedLabel?: string | null;
}

export default function LineAreaChart(props: LineAreaChartProps) {
  const {
    data,
    loading = false,
    width = 640,
  ...
  const [hover, setHover] = useState<LineAreaChartPointWithCoords | null>(null);
  const handleHover = (pt: LineAreaChartPointWithCoords | null) => {
    setHover(pt);
    onHover?.(pt);
  };
  const handleClick = (pt: LineAreaChartPointWithCoords) => {
    onSelectPoint?.(pt);
  };
  ...
  const points: LineAreaChartPointWithCoords[] = parsed.map((p, i) => {
    const x = padX + i * step;
  ...
  const stats: LineAreaChartStats = {
    min,
    max,
    deltaAbs,
    deltaPct,
  ...
  return (
    <div className="max-w-full overflow-x-auto space-y-3">
      ...
      >
        ...
        {points.map((pt, i) => {
          const isSelected =
            selectedLabel != null && selectedLabel === pt.label;
          const isHover = hover != null && hover.label === pt.label;
          return (
            <g key={i}>
              <rect
                x={pt.x - step / 2}
        ...
        {!disableTooltip && hover != null && !selectedLabel && (
          <g>
            <rect
              x={hover.x - 60}
              y={hover.y - 48}
      ...
      {footerRender != null && footerRender(points, stats)}
    </div>
  );
}

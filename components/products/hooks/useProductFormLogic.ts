...
export function useProductFormLogic({
  ...
  }>({ custo_medio: null, ultimo_custo: null });
  const [suggestedPreco, setSuggestedPreco] = React.useState<number | null>(null);
  const [suggestedOrigin, setSuggestedOrigin] = React.useState<"custo_medio" | "ultimo_custo" | null>(null); // 'custo_medio' | 'ultimo_custo' | null
  const [estoqueHint, setEstoqueHint] = React.useState<number | null>(null);

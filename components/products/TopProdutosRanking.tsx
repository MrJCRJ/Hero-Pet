...
export function TopProdutosRanking({ data, onNavigate, showHistory = true }) {
  ...
  if (!rows.length)
    return <div className="text-sm opacity-70">Sem dados de ranking.</div>;
  ...
  function handleClick(id) {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("navigate:produtos", { detail: { q: `#${id}` } }),
      );
    } catch (_) {
      /* noop */
    }
    ...
    <div className="space-y-4 text-sm">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold">
          Top produtos por lucro (limite {limitUsed})
        </h3>
        <div className="text-[11px] opacity-60">
          Margens baseadas em COGS reconhecido
        </div>
      </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.map((r) => (
              <tr
                key={r.produto_id}
                className="border-t last:border-b-0 hover:bg-[var(--color-bg-secondary)] cursor-pointer"
                onClick={() => handleClick(r.produto_id)}
                title="Filtrar lista pelo ID do produto"
              >
                <td className="px-2 py-1" title={r.nome}>
                  {truncate(r.nome, 48)}
                </td>
                ...
                <td className="px-2 py-1 text-right">
                  {Number(r.margem || 0).toFixed(1)}%
                </td>
                <td className="px-2 py-1 text-right">
                  {Number(r.quantidade || 0)}
                </td>
                <td className="px-2 py-1 text-right">
                  {formatBRL(r.lucro_unitario)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {showHistory && months.length > 0 && (
        <div>
            ...
            <table className="w-full text-[11px] min-w-[600px]">
              <thead className="bg-[var(--color-bg-secondary)]">
                <tr>
                  <th className="text-left px-2 py-1">Produto</th>
                  {months.map((m) => (
                    <th key={String(m)} className="text-right px-2 py-1">
                      {String(m)}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr
                    key={r.produto_id}
                    className="border-t last:border-b-0 hover:bg-[var(--color-bg-secondary)]"
                  >
                    <td className="px-2 py-1" title={r.nome}>
                      {truncate(r.nome, 40)}
                    </td>
                    {months.map((mo) => {
                      ...
                      return (
                        <td
                          ...
                          title={pt ? `Margem ${margem}%` : "Sem vendas"}
                        >
          ...
          <div className="text-[11px] opacity-60 mt-2">
            Lucro por mês (— sem vendas). Tooltip mostra margem.
          </div>
        </div>
      )}
      <div className="text-[11px] opacity-60 leading-snug">
        Lucro = Receita - COGS. Margem% = Lucro / Receita. Lucro Unitário =
        Lucro / Qtd.
      </div>
    </div>
  );

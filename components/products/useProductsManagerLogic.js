...
export function useProductsManagerLogic() {
  const { rows, loading, query, setQ, setCategoria, setAtivo, refresh } = useProducts();
  const { push } = useToast();
  ...
  const visibleRows = onlyBelowMin ? rows.filter(p => {
  ...
  function openNew(prefill) { setEditing(prefill || null); setShowModal(true); }
  function openEdit(item) { setEditing(item); setShowModal(true); }
  function openActions(item) { setActionTarget(item); setShowActionsModal(true); }
  function openDetails(item) { setDetailTarget(item); setShowActionsModal(false); setShowDetailModal(true); fetchCostHistory(item.id); }

  async function fetchCostHistory(produtoId) {
    try {
      setDetailLoading(true);
      const resp = await fetch(`/api/v1/produtos/${produtoId}/custos_historicos?months=12`);
      if (!resp.ok) { setCostHistory([]); return; }
      const json = await resp.json();
      let items = [];
      if (Array.isArray(json)) items = json; else if (Array.isArray(json?.data)) items = json.data;
      items = items.slice().sort((a, b) => String(a.month).localeCompare(String(b.month)));
  ...
  async function handleSubmit(data) {
    try {
      ...
      if (!res.ok) { const text = await res.text(); throw new Error(text || MSG.PROD_SAVE_ERROR); }
      setShowModal(false); setEditing(null); await refresh();
    } catch (e) { toastError(push, e, MSG.PROD_SAVE_ERROR); } finally { setSubmitting(false); }
  }

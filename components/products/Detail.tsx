...
export function ProductDetail({ open, onClose, product }) {
  const [saldos, setSaldos] = useState<Record<string, unknown> | null>(null);
            ...
            <section className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-3 rounded-md border border-[var(--color-border)]">
                ...
                <div className="text-lg font-semibold">
                  {loadingSaldos ? "…" : String(saldos?.saldo ?? "-")}
                </div>
              </div>
              <div className="p-3 rounded-md border border-[var(--color-border)]">
                <div className="text-xs text-[var(--color-text-secondary)]">
                ...
                <div className="text-lg font-semibold">
                  {loadingSaldos ? "…" : formatBRL(saldos?.custo_medio as number | undefined)}
                </div>
              </div>
              <div className="p-3 rounded-md border border-[var(--color-border)]">
                <div className="text-xs text-[var(--color-text-secondary)]">
                ...
                <div className="text-lg font-semibold">
                  {loadingSaldos ? "…" : formatBRL(saldos?.ultimo_custo as number | undefined)}
                </div>
              </div>
            </section>
              ...
              <div className="border border-[var(--color-border)] rounded-md overflow-hidden">
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {movs.map((m, i) => (
                      <tr
                        key={String((m as { id?: unknown }).id ?? i)}
                        className="border-t border-[var(--color-border)]"
                      >
                        <td className="p-2">
                          {formatDateTimeBR(m.data_movimento as string)}
                        </td>
                        <td className="p-2">{String(m.tipo ?? "")}</td>
                        <td className="p-2">{String(m.quantidade ?? "")}</td>
                        <td className="p-2">
                          {m.valor_unitario != null
                            ? formatBRL(m.valor_unitario as number)
                            : "-"}
                        </td>
                        <td className="p-2">
                          {m.valor_total != null
                            ? formatBRL(m.valor_total as number)
                            : "-"}
                        </td>
                        <td className="p-2">{String(m.documento ?? "-")}</td>
                        <td className="p-2">{String(m.observacao ?? "-")}</td>
                      </tr>
                    ))}
                    {!movs.length && (
                      <tr>

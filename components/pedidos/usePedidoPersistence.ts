...
interface UsePedidoPersistenceParams {
  ...
  migrarFifo?: boolean;
  setCreated: (v: Record<string, unknown> | null) => void;
  push: (msg: string, opts?: { type?: string; assertive?: boolean }) => void;
  onSaved?: (v?: Record<string, unknown>) => void;
  onCreated?: (v?: Record<string, unknown>) => void;
...
export function usePedidoPersistence({
  ...
  async function handleSubmit(canSubmit: boolean) {
    if (!canSubmit) return;
    setSubmitting(true);
    try {
      const payloadBase = buildBase();
      ...
      if (isEdit) {
        ...
        emitInventoryChanged({
          ...
          source: "order-post",
          orderId:
            result.id != null ? Number(result.id) : undefined,
        });

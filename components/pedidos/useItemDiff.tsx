...
interface DiffItem {
  ...
  desconto_unitario?: unknown;
}

export function useItemDiff(
  itens: DiffItem[],
  originalItens: DiffItem[],
  ...
  const getItemChanges = useMemo(() => {
    return function () {
      ...
      const removed = originalItens.filter(
        (it) => it.produto_id != null && !currentIds.has(it.produto_id),
      );
  ...
  const getItemDiffClass = useMemo(() => {
    ...
    return function (item: DiffItem): string {
  ...
  const getItemDiffIcon = useMemo(() => {
    const changes = getItemChanges();
    const DiffIcon = function (item: DiffItem): React.ReactNode {

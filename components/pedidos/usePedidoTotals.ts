...
export interface UsePedidoTotalsParams {
  itensRef: React.RefObject<unknown[] | null>;
  tipoRef: React.RefObject<string>;
  numeroPromissoriasRef: React.RefObject<number | string | null>;
  setValorPorPromissoria: React.Dispatch<React.SetStateAction<number>>;
  freteRef?: React.RefObject<number | string | null | undefined>;
}
...
export function usePedidoTotals({
  ...
  freteRef,
}: UsePedidoTotalsParams) {
  const computeItemTotal = React.useCallback(
    (it: Record<string, unknown>) => computeItemTotalPure(it),
    []
  );

  const subtotal = React.useCallback(() => {
    const itens = (itensRef.current || []) as Record<string, unknown>[];

...
export interface PedidoFormResumoLucroProps {
  itens: Array<Record<string, unknown>>;
  totalItens: number;
  percentuaisRefParsed: number[];
  percRefInput: string;
  setPercRefInput: (_v: string) => void;
}

export function PedidoFormResumoLucro({
  itens,
  totalItens,
  percentuaisRefParsed,
  percRefInput,
  setPercRefInput,
}: PedidoFormResumoLucroProps) {
  ...
  return (
    <div className="flex flex-col items-end gap-1 text-xs">
      ...
      <div className="opacity-60 text-[10px] leading-snug max-w-[560px] text-right">
        ...
        percentuais em &quot;Ref %&quot; (máx 4). Lucro - X% = Lucro total -
        Comissão X%.
      </div>

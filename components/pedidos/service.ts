...
export async function fetchSaldo(produtoId: number): Promise<number | null> {
  try {
...
export async function fetchSaldoDetalhado(
  produtoId: number
): Promise<{
  saldo: number | null;
...
export async function fetchSaldoFifoDetalhado(
  produtoId: number
): Promise<{
  quantidade_total: number | null;
  custo_medio_fifo: number | null;
...
export async function fetchEntities({
  ...
  q: string;
  tipo?: string;
}): Promise<Array<{ id: number; label: string; name: string }>> {
  ...
  return (data as Array<{ id: number; name: string; entity_type: string }>).map((e) => ({
    id: e.id,
...
export async function migrateOrderToFIFO(
  orderId: number
): Promise<Record<string, unknown>> {
  const res = await fetch(`/api/v1/pedidos/${orderId}`, {
    method: "PUT",
...
export async function fetchLastPurchasePrice(
  produtoId: number
): Promise<number | null> {
  try {
    const res = await fetch(

...
interface UsePromissoriasScheduleParams {
  numeroPromissorias: number;
  onNumeroPromissoriasChange: (n: number, valorCalculado?: number) => void;
  dataPrimeiraPromissoria: string;
  onDataPrimeiraPromissoriasChange: (v: string) => void;
  totalLiquido: number;
  frequenciaPromissorias: string;
  intervaloDiasPromissorias: number;
  promissoriaDatas: string[];
  onPromissoriaDatasChange: (v: string[]) => void;
  promissoriasMeta?: { paidSeqs?: number[]; overdueSeqs?: number[] } | null;
}
...
export function usePromissoriasSchedule({
  ...
  promissoriasMeta,
}: UsePromissoriasScheduleParams) {
  const warnedPaidRef = React.useRef<Set<number>>(new Set());
  const { push } = useToast();
  ...
  const handleManualDateEdit = React.useCallback(
    (idx: number, e: React.ChangeEvent<HTMLInputElement> | React.FormEvent) => {
      const newValue =

...
export interface PedidoFormOrcamentoCompraProps {
  itens: FormItem[];
  freteTotal: number | string;
}

export function PedidoFormOrcamentoCompra({
  itens,
  freteTotal,
}: PedidoFormOrcamentoCompraProps) {
  const [precosVenda, setPrecosVenda] = React.useState<Record<number, string>>(
    () => ({})
  );
  const [percVendedor, setPercVendedor] = React.useState<
    Record<number, string>
  >(() => ({}));

  const handleChangePreco = (pid: number, value: string) => {
    setPrecosVenda((prev) => ({ ...prev, [pid]: value }));
  };
  const handleChangePercVendedor = (pid: number, value: string) => {
  ...
  const agregados = React.useMemo(() => {
    ...
    for (const it of itens) {
      ...
      if (!(Number.isFinite(custoUnit) && custoUnit != null && custoUnit > 0))
        continue;
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {agregados.map((row) => {
            ...
            const custoMedio = row.custoMedio ?? 0;
            const lucroBruto =
              Number.isFinite(pvRaw) && pvRaw > 0
                ? (pvRaw - custoMedio) * row.qtd
                : null;
            const comissao =
            ...
            return (
              ...
              >
                ...
                >
                  {formatBRL(row.custoMedio ?? 0)}
                </td>
                <td
                  className="p-2 text-right"
                  data-testid={`orc-ct-${row.produto_id}`}
                >
                  {formatBRL(row.custoTotalProduto ?? 0)}
                </td>
                ...
                >
                  {lucroLiquido == null
                    ? "—"
                    : formatBRL(Number(lucroLiquido.toFixed(2)))}
                </td>
              </tr>
            );
          })}
              ...
              <td className="p-2 text-right" data-testid="orc-total-lucro">
                {(() => {
                  ...
                  for (const row of agregados) {
                    ...
                    const custoMedio = row.custoMedio ?? 0;
                    const lucroBruto = (pvRaw - custoMedio) * row.qtd;

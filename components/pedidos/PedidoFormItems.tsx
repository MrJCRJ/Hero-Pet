...
import { computeItemTotal as defaultComputeItemTotal, computeFreteShares } from "./utils";
...
type ProdutoOption = { id: string | number; label: string };

export interface PedidoFormItemsProps {
  ...
  onRemoveItem: (_idx: number) => void;
  tipo: string;
  computeItemTotal?: (
    item: Record<string, unknown>
  ) => number | null;
  getItemDiffClass: (item: Record<string, unknown>) => string;
  getItemDiffIcon: (item: Record<string, unknown>) => React.ReactNode;
  getItemChanges: () => { removed: Array<Record<string, unknown>> };
  originalItens: Array<Record<string, unknown>>;
  editingOrder?: Record<string, unknown> | null;
  productModalIndex: number | null;
  onSetProductModalIndex: (_idx: number | null) => void;
  fetchProdutos: (_q: string) => Promise<ProdutoOption[]>;
  freteTotal: number | string;
  setFreteTotal: (_v: number | string) => void;
}

export function PedidoFormItems(props: PedidoFormItemsProps) {
  const {
    itens,
    onUpdateItem,
    onAddItem,
  ...
  return (
    <div className="mt-6">
        ...
        changes.removed.length > 0 && (
          <div className="mb-4 p-3 border border-red-500 bg-red-50 dark:bg-red-900/20 rounded-md">
            ...
            {changes.removed.map((removedItem, idx) => (
              ...
              >
                •{" "}
                {(removedItem.produto_label as string) ||
                  `Produto ID: ${removedItem.produto_id}`}{" "}
                - Qtd: {String(removedItem.quantidade ?? "")}- Preço:{" "}
      ...
      {productModalIndex != null &&
        Number.isInteger(productModalIndex) &&
        productModalIndex >= 0 && (
        <SelectionModal<ProdutoOption>
          ...
          onSelect={async (it) => {
            ...
            if (it && Number.isInteger(targetIndex)) {
              onUpdateItem(targetIndex, {
                ...
                produto_saldo: null,
                custo_carregando: tipo === "VENDA",
              });
              if (tipo === "VENDA") {
                const prodId = Number(it.id);
                Promise.all([
                  fetchSaldoDetalhado(prodId),
                  fetchSaldoFifoDetalhado(prodId),
                ])
                  .then(([det, fifo]) => {
                    const saldo = det.saldo;
                    const cmf = fifo.custo_medio_fifo;
                    const custoFifo =
                      cmf != null && Number.isFinite(cmf) && cmf > 0
                        ? cmf
                        : null;
                    const baseLegacy = (() => {
                ...
                try {
                  const lastPrice = await fetchLastPurchasePrice(Number(it.id));
                  if (lastPrice != null) {

...
export function usePedidoPromissorias(
  ...
  function hydrateFromEditing(order: Record<string, unknown> | null) {
    if (!order) return;
    if (Array.isArray(order.promissorias) && order.promissorias.length) {
      ...
      const paidSeqs = order.promissorias
        .filter((p) => p.paid_at)
        .map((p) => p.seq);
      const today = new Date();
  ...
  function computePromissoriasMismatch(totalLiquido: number) {

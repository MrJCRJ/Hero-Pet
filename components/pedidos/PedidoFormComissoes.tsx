...
export interface PedidoFormComissoesProps {
  comissoes: ComissaoLinha[];
  setComissoes: React.Dispatch<React.SetStateAction<ComissaoLinha[]>>;
  computeOrderTotalEstimate: () => number;
  lucroBruto: number;
}
...
export function PedidoFormComissoes({
  ...
  function updatePercentual(value: string) {
    setComissoes((prev) => prev.map((l) => ({ ...l, percentual: value })));
  }

  function calcularLinha(l: ComissaoLinha): number {

...
export interface PedidoItemRowProps {
  ...
  idx: number;
  tipo: string;
  computeItemTotal: (item: Record<string, unknown>) => number | null;
...
export function PedidoItemRow({
  ...
  return (
    <tr className={`${getItemDiffClass(it) || ""}`}>
      <td className="px-2 py-2 max-w-[240px]">
        <div className="flex items-center gap-2">
          <span className="font-medium truncate">
            {(it.produto_label as string) || "Produto não selecionado"}
          </span>
          {getItemDiffIcon(it)}
        </div>
      </td>
      <td className="px-2 py-2 text-right whitespace-nowrap">
        {formatQty(it.quantidade as string)}
      </td>
      <td className="px-2 py-2 text-right whitespace-nowrap">

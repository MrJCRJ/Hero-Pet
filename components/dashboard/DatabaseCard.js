...
const DatabaseIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 inline mr-2 text-blue-500">
...
function mapLatency(latency) {
  return {
    "Primeira query": {
      ...
      status: latency.first_query < 30 ? "good" : latency.first_query < 100 ? "warning" : "bad"
    },
    "Segunda query": {
      value: `${latency.second_query.toFixed(2)} ms`,
      status: latency.second_query < 30 ? "good" : latency.second_query < 100 ? "warning" : "bad"
    },
    "Terceira query": {
      value: `${latency.third_query.toFixed(2)} ms`,
      status: latency.third_query < 30 ? "good" : latency.third_query < 100 ? "warning" : "bad"
    },
  };
}

export function DatabaseCard({ database, compact = false }) {
  const connectionPercentage = (database.current_connections / database.max_connections) * 100;
  const connectionStatus = connectionPercentage < 70 ? "good" : connectionPercentage < 90 ? "warning" : "bad";
    ...
    <div className="database-card">
      ...
      <div className="space-y-3">
        ...
        <div className="w-full bg-gray-200 rounded-full h-1.5">
          <div
            className={`h-1.5 rounded-full ${connectionStatus === "good" ? "bg-green-500" :
